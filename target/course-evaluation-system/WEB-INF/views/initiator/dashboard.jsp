...
<head>
  ...
  <title>Initiator Dashboard — CES</title>
...
<div class="app-wrapper">
  <aside class="sidebar">
    ...
    <nav class="sidebar-nav">
      <a href="${pageContext.request.contextPath}/initiator/dashboard" class="nav-item active"><span class="nav-icon"><i class="fas fa-tachometer-alt"></i></span> Dashboard</a>
  ...
  <div class="main-content">
    <header class="top-header">
      <span class="header-title">Initiator Dashboard</span>
    ...
    <div class="page-body">
      ...
      <div class="stats-grid">
        <div class="stat-card" style="--stat-color:#4f46e5;--stat-bg:rgba(79,70,229,0.15);">
          <div class="stat-icon"><i class="fas fa-poll" style="color:#4f46e5;"></i></div>
          <div><div class="stat-value">${totalSurveys}</div><div class="stat-label">Total Surveys</div></div>
        </div>
        <div class="stat-card" style="--stat-color:#10b981;--stat-bg:rgba(16,185,129,0.15);">
          ...
          <div><div class="stat-value">${activeSurveys}</div><div class="stat-label">Active Surveys</div></div>
        </div>
      </div>

      <div class="card">
        <div class="card-header">
          <span class="card-title">My Surveys</span>
          <a href="${pageContext.request.contextPath}/initiator/surveys/new" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> Create</a>
        </div>
            ...
            <tbody>
              <c:forEach var="s" items="${surveys}">
                <tr>
                  ...
                  <td><span style="color:var(--primary-light);">${s.course.code}</span></td>
                  <td><span class="badge badge-${s.status.name().toLowerCase()}">${s.status}</span></td>
                  <td style="text-align:center;">${s.responseCount}</td>
                  <td><span class="badge badge-info" style="font-size:10px;">${s.accessType}</span></td>
                  <td>
              ...
              <c:if test="${empty surveys}">
                <tr><td colspan="6" class="text-center text-muted" style="padding:40px;">
                  No surveys yet. <a href="${pageContext.request.contextPath}/initiator/surveys/new">Create one!</a>
                </td></tr>
              </c:if>
            </tbody>
          </table>
